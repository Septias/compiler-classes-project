...
def patch_instruction(i: src.Instr) -> tgt.Block:
    ...
    match i:
        ...
        case src.Call(src_, ty):
            # BEGIN
            match src_:
                case src.Const(_, _) as c:
                    out += ilist(tgt.IInstr1("li", t0, patch_const(c)))
                    address = t0
                ...
                case int(c):
                    match op:
                        ...
                        case "sub":
                            out += ilist(tgt.IInstr2("addi", rd, rs1, -c))
                        case "xor":
                            out += ilist(tgt.IInstr2("xori", rd, rs1, c))
                        case "slt":
                            out += ilist(tgt.IInstr2("slti", rd, rs1, c))
                        case "sll":
                            out += ilist(tgt.IInstr2("slli", rd, rs1, c))
                        case "srl":
                            out += ilist(tgt.IInstr2("srli", rd, rs1, c))
                        case "sltu":
