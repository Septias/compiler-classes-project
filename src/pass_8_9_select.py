    ...
    match s:
        ...
        case src.SAssign(lhs, src.EOp2Arith(e1, op, e2)):
            ...
            match op:
                case "+":
                    return ilist(
                        tgt.Instr2("add", lhs_out, select_atom(e1), select_atom(e2))
                    )
                case "-":
                    return ilist(
        ...
        case src.SAssign(lhs, src.EOp1(op, e)):
            ...
            match op:
                case "not":
                    return ilist(
                        tgt.Instr2("sub", lhs_out, tgt.Const(2, '64bit'), select_atom(e))
                    )
                case "-":
                    return ilist(
                        tgt.Instr2("sub", lhs_out, tgt.Const(0, '64bit'), select_atom(e))
                    )
        ...
        case src.STryEnter(exc_handler, saved_exc):
            return ilist(
                # Save previous exception data
                tgt.Move(saved_exc.sp, tgt.Offset(Label("exc_sp"), 0)),
                tgt.Move(saved_exc.fp, tgt.Offset(Label("exc_fp"), 0)),
                tgt.Move(saved_exc.handler, tgt.Offset(Label("exc_handler"), 0)),
                # Set new exception data
                tgt.Move(tgt.Offset(Label("exc_sp"), 0), sp),
                tgt.Move(tgt.Offset(Label("exc_fp"), 0), fp),
                tgt.Move(tgt.Offset(Label("exc_handler"), 0), exc_handler),
            )
        case src.STryExit(saved_exc):
            return ilist(
                # Restore previous exception data
                tgt.Move(tgt.Offset(Label("exc_sp"), 0), saved_exc.sp),
                tgt.Move(tgt.Offset(Label("exc_fp"), 0), saved_exc.fp),
                tgt.Move(tgt.Offset(Label("exc_handler"), 0), saved_exc.handler),
            )
        case src.SExcEnter(x):
            return ilist(
                tgt.Move(x, a0)
            )
        case src.SRaise(e):
            return ilist(
                # Restore previous exception data
                tgt.Move(sp, tgt.Offset(Label("exc_sp"), 0)),
                tgt.Move(fp, tgt.Offset(Label("exc_fp"), 0)),
                tgt.Move(a0, select_atom(e)),
                tgt.Move(t0, tgt.Offset(Label("exc_handler"), 0)),
                tgt.JumpIndirect(t0),
            )
        case _:
