...
def conv_ass_stmt(AF: set[Id], s: src.Stmt) -> tgt.Stmt:
    match s:
        ...
        case src.SIf(e, b1, b2):
            ...
            b2 = conv_ass_stmts(AF, b2)
            return tgt.SIf(e, b1, b2)
        case src.SWhile(e, b):
            e = conv_ass_expr(AF, e)
            b = conv_ass_stmts(AF, b)
        ...
        case src.STry(body, x, handler):
            body = conv_ass_stmts(AF, body)
            handler = conv_ass_stmts(AF, handler)
...
def free_in_lambda_stmt(s: src.Stmt) -> set[Id]:
    match s:
        ...
        case src.SRaise(e):
            return free_in_lambda(e)
        case src.STry(body, x, handler):
            return free_in_lambda_stmts(body) | (free_in_lambda_stmts(handler) - { x })
